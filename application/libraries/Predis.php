 ...
 * CodeIgniter Redis
 *
 * A CodeIgniter library to interact with Redis degang@leju.com 修改版
 *
 * @package        	CodeIgniter
 * @category    	Libraries
 * @author        	Joël Cox degang@leju.com修改版
 * @version			v0.4
...
class Predis 
{
	...
	public function __construct($params = array())
	{
        ...
        //添加一致性hash
        if (isset($params['redis']['flexihash']) && $params['redis']['flexihash'] == 1)
        {
            $this->_ci->load->library('Flexihash');
            $this->_ci->flexihash->addTargets(array_keys($params['redis']['list']));
        }
    }
    ...
    private function _connection()
    {
        ...
        {
            //获取命中的hash
            list($config, $flexihash_one) = $this->_get_hash($params);
        }
        else
        {
        ...
        if ( ! $this->_connection)
        {
            ...
            if (isset($params['redis']['flexihash']) && $params['redis']['flexihash'] == 1)
            {
                unset($this->_params['redis']['list'][$flexihash_one]);
                $this->_ci->flexihash->removeTarget($flexihash_one);
            }
            else
                show_error('Could not connect to Redis at ' . $config['host'] . ':' . $config['port']);

            if (count($this->_params['redis']['list']) == 0)
                show_error('Could not connect to Redis at ' . $config['host'] . ':' . $config['port']);
            else
            {
			    log_message('debug', 'Could not connect to redis at : ' . $config['host'] . ':' . $config['port']);
                $this->_connection();
            }
        }

        //echo $flexihash_one.'<hr />';
     ...
     * @return array 
     */
    private function _get_hash($params)
    {
        $flexihash_one = $this->_ci->flexihash->lookup($this->key); 
        $config = $params['redis']['list'][$flexihash_one];
        return array($config, $flexihash_one);
    }
	/**
	 * Call
	 *
	...
	{
        $this->key = isset($arguments[0]) && $arguments[0] ? $arguments[0] : 0;
        $this->_connection();
